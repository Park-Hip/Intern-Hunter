...
- `POST /agent/ask` returns a typed database-agent envelope with a profanity-only guardrail backed by `src/agents/bad_words.txt`, a preview stub path, and a runtime-backed allowed path with a top-level `answer` field, intentional short in-process session memory, YAML-backed runtime prompts, and request tracing metadata when the configured local Ollama model is responsive
